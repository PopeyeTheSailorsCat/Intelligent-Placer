...
Результат неплохо, однако нужно будет дописать обработку случая, когда не найдены предметы и избавиться от примера, где засвечен угол экрана.


Теперь попробуем найти на изображении многоуольник.
Воспользуемся детектором границ Canny. Добавим пост обработку - воспользуемся данными о найденных предметах, чтоб закрасить их bboxами область вокруг них. Кроме того, сделаем закрытие,
