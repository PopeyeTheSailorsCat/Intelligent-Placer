...
# План работы
* Шаг 1: Извлечь из изображения структуру фигуры, ее местоположение и картинки предметов: 
  * Перевести изображение в grayscale используя rgb2gray
  * Найты bboxы фигуры и предметов.
    * bboxы предметов находим с помощью маски полученной min threshold,  и последующим применением sk_measure_label и regionprops чтобы получить данные о связанных компонентах.
    * bbox фигуры находим с помощью детектора canny.
    * Canny найдет не только границы фигуры, но и границы предметов. Воспользуемся bboxами предметов, закрасив области маски полученной canny. 
    * Выполним binnary_fill, чтобы заполнить фигуру. Сделаем binary_opening на случай, если остался какой то шум.
    * Найдем bbox фигуры аналогично bboxам предметов
  * С помощью bboxов предметов вырежем их из исходного изображения и приведем их к формату, который сможет обработать обученная CNN
  * С помощь bbox фигуры, вырежем ее из исходного изображения, применим к результату canny, и заполним полученные границы. Таким образом мы получили структуру фигуры.
* Шаг 2: Классифицируем предметы с изображения, используя вырезанные фигуры:
  * Получив на с предыдущего шага картинки полученных предметов, отправим их в часть программы, ответственной за нейросеть.
  * Скомпилируем нашу модель и загрузим из репозитория сохраненные веса.
  * С помощью модели предсказываем вероятности классов предметов на изображении, которые мы вырезали
* Шаг 3: Располагаем предметы на фигуре.
  * По результатам предсказания CNN, делаем классификацию предметов на изображении.
  * По результата классификации загружаем структуры этих предметов из репозитория.
  * Для каждой структуры каждого предмета из классифицированных, начинаем процедуру размещения на фигуре:
    * Имеем маску фигуры и маску предмета, проходим с некоторым шагом маской предмета над маской фигуры.
    * Выполняем операцию логического И между маской предмета и участком маски фигуры под ним.
    * Если в результате количество истинных значений будет равняться площади структуры предмета, то мы говорим, что этот предмет размещается здесь.
    * Вычитаем из структуры фигуры, которую покрывает маска предмета, структуру предмета (понятно что важно лишь вычитание Истины из Истины на масках)
    * Возвращаем координаты, где располагается на структуре фигуры этот предмет, и новую структуру предмета, чтобы при расположении нового предмета не покрыть предыдущий.
...
Сделаем максимально близко к нормали фотоснимки объектов, на расстоянии, которое будет наиболее ожидаемым в процессе работы. Полученный результат будем использовать после классификации, как структуру классифицированного объекта:
...
* Избавиться от элементов, зависящих от расширения фотографии
